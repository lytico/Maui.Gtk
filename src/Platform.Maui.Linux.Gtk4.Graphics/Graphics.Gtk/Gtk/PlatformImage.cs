...
public class PlatformImage : IImage {
    public PlatformImage (GdkPixbuf.Pixbuf pix) {
        _pixbuf = pix;
    }

    private GdkPixbuf.Pixbuf? _pixbuf;

    // https://developer.gnome.org/gdk-pixbuf/stable/gdk-pixbuf-The-GdkPixbuf-Structure.html
    public GdkPixbuf.Pixbuf? NativeImage => _pixbuf;
    ...
    public static IImage FromStream (Stream stream, ImageFormat formatHint) {
        // Read stream into a byte array
        byte[] buffer = new byte[stream.Length];
        stream.ReadExactly (buffer); // Reads entire stream into buffer
        using var glibBytes = GLib.Bytes.New (buffer);

        using var gioStream = new MemoryInputStream ();
        gioStream.AddBytes (glibBytes);
        var pix = GdkPixbuf.Pixbuf.NewFromStream (gioStream, null);
