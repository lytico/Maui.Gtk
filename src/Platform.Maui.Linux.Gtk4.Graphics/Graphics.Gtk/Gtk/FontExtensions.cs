...
public static class FontExtensions {
    ...
    static Pango.FontDescription SystemFontDescription => _systemFontDescription ??= SystemContext.GetFontDescription ();
    ...
    public static Pango.FontDescription ToFontDescription (this IFont it) {
        if (it is not { } || string.IsNullOrEmpty (it?.Name))
            return SystemFontDescription;

        var fd = FontDescription.New ();
        fd.SetStyle (it.StyleType.ToPangoStyle ());
        fd.SetWeight (it.Weight.ToPangoWeight ());
        fd.SetFamily (it.Name);
